...
async def send_alert(payload: dict):
    ...
    """
    print(f"[MCP] 正在向 {payload.get('target')} 发送预警...")
    ...
    async with async_session_factory() as session:
        try:
            ...
            print(f"[MCP] 预警发送成功！风险事件已记录，ID: {new_risk_event.id}")

        except Exception as e:
            # 发生异常时回滚事务，防止脏数据
            await session.rollback()
            print(f"[MCP] 预警记录写入数据库失败: {e}")
            raise  # 向上抛出异常，以便 TaskQueue 捕获并进行重试逻辑


async def create_case(payload: dict):
    """MCP工具：创建心理干预个案"""
    print(f"[MCP] 正在为学生 {payload.get('user_id')} 创建干预个案...")
